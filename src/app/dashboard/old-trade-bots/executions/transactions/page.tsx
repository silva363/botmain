...
export default function Transactions() {
  ...
  const botType = searchParams.get("type") ? searchParams.get("type") : null;
    ...
    const getTransactions = async () => {
      try {
        const response = await axios.post(
          `${process.env.NEXT_PUBLIC_BASE_APP_URL}/api/old-trade-bot/executions/transactions`,
          {
            authtoken: token,
  ...
  }, [isLoading, isLoadingTokenList, tokenList, selectedTokenSymbol, idParams, pagination, token]);
  ...
  const handleAirdropClick = async () => {
    if (!confirm("Você tem certeza que deseja iniciar o airdrop?")) {
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BASE_APP_URL}/api/old-trade-bot/executions/transactions/airdrop`, { id, authtoken: token },
      );
        ...
        <>
          <div className="header-container">
            <div className="header-title">
              ...
              <h5>Estratégia: {botType == 'buy' ? 'Compra' : botType == 'sell' ? 'Venda' : ''}</h5>
            </div>
          ...
          <div style={{ marginTop: "25px" }}>
            <DivBot>
              ...
              <span style={{
                backgroundColor: `${activeAirdrop == 1 ? "#ffff00" : "#1E2021"}`,
                color: `${activeAirdrop == 1 ? "black" : "whtiesmoke"}`
              }}>
                <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100%" }}>
                  {activeAirdrop != -1 ? (
                    activeAirdrop == 1 ? (
                      <p style={{ textTransform: "uppercase", letterSpacing: "2px" }}><b>Airdrop em andamento</b></p>
                    ) : (
                      <button
                        style={{ backgroundColor: "transparent", color: "whitesmoke", width: "100%", height: "100%", border: "none", textTransform: "uppercase", letterSpacing: "2px" }}
                        onClick={() => handleAirdropClick()}
                      >
                        <b>Realizar Airdrop</b>
                      </button>
                    )
                  ) : (
