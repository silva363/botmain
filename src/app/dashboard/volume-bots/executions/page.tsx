...
export default function Executions() {
    ...
    const getExecutions = async () => {
      try {
        const response = await axios.post(
          `${process.env.NEXT_PUBLIC_BASE_APP_URL}/api/volume-bot/list/executions`,
          {
            authtoken: token,
            uuid: idParams,
            page: pagination,
  ...
  const handleReRun = async (id: number) => {
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BASE_APP_URL}/api/volume-bot/list/executions/reRun`,
  ...
  const pageTransactions = (id: number) => {
    push(`/dashboard/volume-bots/executions/transactions?id=${id}&uuid=${idParams}&name=${botName}&ghost_wallets=${ghostWallets}`);
  };
