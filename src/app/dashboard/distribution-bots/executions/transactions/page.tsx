...
import { getMinifiedAddress } from "@/app/utils/data";
    ...
    const getTransactions = async () => {
      try {
        const response = await axios.post(
          `${process.env.NEXT_PUBLIC_BASE_APP_URL}/api/distribution-bot/list/executions/transactions`,
          {
            authtoken: token,
          ...
          <div style={{ marginTop: "25px" }}>
            <DivBot>
              ...
              <span style={{ backgroundColor: "#FF5733", color: "whitesmoke" }}>
                ...
                <p>
                  {reponseTransactions ? (
                    reponseTransactions.data?.details?.matic_fails ? (
                      reponseTransactions.data.details.matic_fails
                    ) : (
                      0
                    )
              ...
              <span style={{ backgroundColor: "#FF5733", color: "whitesmoke" }}>
                ...
                <p>
                  {reponseTransactions ? (
                    reponseTransactions.data?.details?.selected_token_fails ? (
                      reponseTransactions.data.details.selected_token_fails
                    ) : (
                      0
                    )
