...
export default function Executions() {
    ...
    const getExecutions = async () => {
      try {
        const response = await axios.post(
          `${process.env.NEXT_PUBLIC_BASE_APP_URL}/api/trade-bot/executions`,
          {
            authtoken: token,
            uuid: idParams,
            page: pagination,
  ...
  const handleReRun = async (id: number) => {
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BASE_APP_URL}/api/trade-bot/reRun`,
  ...
  const pageTransactions = (id: number) => {
    push(`/dashboard/trade-bots/executions/transactions?id=${id}&uuid=${idParams}&name=${botName}&type=${botType}`);
  };
          ...
          <div style={{ marginTop: "25px" }}>
            <DivBot>
              ...
              <span style={{ backgroundColor: "#1E2021", color: "whitesmoke" }}>
                ...
                <p>
                  {reponseExecutions ? (
                    reponseExecutions.data?.details?.ghost_wallets ? (
                      reponseExecutions.data.details.ghost_wallets
                    ) : (
                      0
                    )
              ...
              <span style={{ backgroundColor: "#28a745", color: "whitesmoke" }}>
                ...
                <p>
                  {reponseExecutions ? (
                    reponseExecutions.data?.details?.matic_input ? (
                      reponseExecutions.data.details.matic_input
                    ) : (
                      0
                    )
              ...
              <span style={{ backgroundColor: "#28a745", color: "whitesmoke" }}>
                ...
                <p>
                  {reponseExecutions ? (
                    reponseExecutions.data?.details?.selected_token_input ? (
                      reponseExecutions.data.details.selected_token_input
                    ) : (
                      0
                    )
              ...
              <span style={{ backgroundColor: "#FF5733", color: "whitesmoke" }}>
                ...
                <p>
                  {reponseExecutions ? (
                    reponseExecutions.data?.details?.matic_pending ? (
                      reponseExecutions.data.details.matic_pending
                    ) : (
                      0
                    )
              ...
              <span style={{ backgroundColor: "#FF5733", color: "whitesmoke" }}>
                ...
                <p>
                  {reponseExecutions ? (
                    reponseExecutions.data?.details?.selected_token_pending ? (
                      reponseExecutions.data.details.selected_token_pending
                    ) : (
                      0
                    )
              ...
              <span style={{ backgroundColor: "transparent", boxShadow: "none" }}></span>
            </DivBot>
          </div>
